                ...
                )
{
                ...
                $action_page="farm_workers_production.php?l=".$limit."&s=".$skip."&sc=".$sort_column."&so=".$sort_order."&re=".$rows_every."&data=".$_GET['data'];//for form submission
...
<head>
    ...
    <title>Farm worker production</title>
    ...
    <hr>
     ...
     <table>
       ...
       // echo $_POST['search_phrase'];
                            $returned_json_decoded= json_decode(send_curl_post(the_api_requests_url."user_fetch_egg_collections_user_id/",'sort_by='.$sort_column.'&sort_order='.$sort_order.'&limit='.$limit.'&skip='.$skip.'&user_id='.$user_data["_id"].'&search_phrase='.$_POST['search_phrase'].'',array("Customkey: ".$_SESSION['token_type']." ".$_SESSION['token'] )),true);//decode
                            ...
                            if($returned_json_decoded["check"]==true)//if login is true
                            {
                                ...
                                foreach ($returned_json_decoded["message"] as $value) 
                                {
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="farm_workers_production_edit.php?<?php echo $action_page_without_page_name?>&data_1=<?php echo base64_encode(json_encode($value))?>">Edit</a></td>
                                        <td><a href="farm_workers_production_delete.php?<?php echo $action_page_without_page_name?>&data_1=<?php echo base64_encode(json_encode($value))?>">Delete</a></td>
                                    </tr>
                                    <?php
