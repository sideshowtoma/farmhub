    ...
    <hr>
     ...
     <table>
        <tr>
            ...
            <th><a href="<?php echo $link_without_sort_column_sort_order?>&sc=date_added&so=<?php echo return_script_order($sort_column,$sort_order,"date_added")?>">Date used</a></th>
        </tr>
        <?php
    
       // echo $_POST['search_phrase'];
                            $returned_json_decoded= json_decode(send_curl_post(the_api_requests_url."user_fetch_inventory_in_out_user_id/",'sort_by='.$sort_column.'&sort_order='.$sort_order.'&limit='.$limit.'&skip='.$skip.'&user_id='.$personal_info["user_id"].'&in_out=out&search_phrase='.$_GET['search_phrase'].'',array("Customkey: ".$_SESSION['token_type']." ".$_SESSION['token'] )),true);//decode
